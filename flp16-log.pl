...
% Parses single line
parseTransition([OldState, ' ', Symbol, ' ', NewState, ' ', Action | []],Parsed) :-
...
simulateTS(Level, Tape, State, Transitions, Output) :-
    ...
    ( Action == 'L', shiftLeft(Tape, State, NewTape)
      ...
      ; Action \= 'L', Action \= 'R', writeSymbol(Tape, State, Action, NewTape) 
    ),
...
% Writes specified symbol on tape at head location
